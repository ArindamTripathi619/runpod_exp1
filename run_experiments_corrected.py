...
def run_experiment_1(n_trials: int, db: Database, config: Config) -> Dict[str, Any]:
    """
    Experiment 1: Baseline LLM Defense - Layer Propagation (RQ1)
    
    Tests 4 configurations to establish baseline and progressive layer activation:
    - Config A: No Defense (baseline vulnerability)
    - Config B: Layer 2 Only (semantic analysis)
    - Config C: Layers 2-3 (semantic + context)
    - Config D: Full Pipeline (complete defense)
    
    Tests 52 prompts (42 attacks + 10 benign) per trial
    Per STATISTICAL_REQUIREMENTS.md
    """
    logger.info("="*80)
    logger.info("EXPERIMENT 1: Baseline LLM Defense - Layer Propagation (RQ1)")
    ...
    experiment_id = "exp1_baseline_defense"
